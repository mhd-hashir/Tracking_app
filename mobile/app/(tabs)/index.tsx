  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      <View style={styles.header}>
        ...
        <TouchableOpacity onPress={signOut} style={{ backgroundColor: '#dc2626', paddingVertical: 8, paddingHorizontal: 16, borderRadius: 8 }}>
          <Text style={{ color: '#fff', fontWeight: 'bold', fontSize: 14 }}>Log Out</Text>
        </TouchableOpacity>
      </View>
      ...
      {loadingRoute ? (
        ...
        <View style={styles.routeCard}>
          ...
          {route.stops.map((stop, index) => {
            ...
            let cardStyle: any = {}; 
            if (status.edited) {
              cardStyle = { backgroundColor: '#e0f2fe', borderColor: '#7dd3fc', borderWidth: 1 }; // Blue
            } else if (status.collected) {
              cardStyle = { backgroundColor: '#dcfce7', borderColor: '#86efac', borderWidth: 1 }; // Green
            } else if (status.remark) {
              cardStyle = { backgroundColor: '#fef9c3', borderColor: '#fde047', borderWidth: 1 }; // Yellow
            }

            return (
            <Link key={stop.id} href={`/shop/${stop.shop.id}` as any} asChild>
              <TouchableOpacity style={[styles.stopItem, cardStyle]}>
